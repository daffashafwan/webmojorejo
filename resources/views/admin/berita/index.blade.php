...
<div class="block">
    ...
    <div class="block-content block-content-full">
        ...
        <div>
            <a href="{{route('admin.berita.tambah')}}" style="text-decoration: none"
                class="btn btn-primary mb-2 text-white">Tambah Berita</a>
        </div>
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td class="d-none d-sm-table-cell">
                        <form action="{{route('admin.berita.ubah-status')}}" method="POST">
                            @csrf
                            <input type="hidden" name="id" value="{{$b->id}}">
                            <input value="<?php echo(($b->status == 'Aktif') ? '1' : '0') ?>" type="hidden" name="gantistatus" id="">
                            <button class="badge form-control btn badge-<?php echo(($b->status == 'Aktif') ? 'success' : 'danger') ?>" type="submit"><?php echo(($b->status == 'Aktif') ? 'Aktif' : 'Tidak Aktif')?></button>
                        </form>
                    </td>
                    <td>
                        <div class="btn-group mr-2 mb-2" role="group" aria-label="Icons Text group">
                            <a href="{{route('admin.berita.lihat-berita', ['id'=>$b->id])}}" type="button"
                                class="text-white btn btn-primary">
                                <i class="fa fa-fw fa-eye"></i>
                            </a>
                            <a href="{{route('admin.berita.edit', ['id'=>$b->id])}}" type="submit"
                                class="text-white btn btn-primary">
                                <i class="fa fa-fw fa-edit"></i>
                            </a>
                            <a bid="{{$b->id}}" data-toggle="modal" data-target="#modal-hapus-popout" type="button"
